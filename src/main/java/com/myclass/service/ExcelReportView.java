...
public class ExcelReportView extends AbstractXlsView {
	...
	protected void buildExcelDocument(Map<String, Object> model, Workbook workbook, HttpServletRequest request,
		...
		List<String[]> report3 = (List<String[]>) model.get("report3");
		...
		// report3
		sheet.addMergedRegion(new CellRangeAddress(rowNum, rowNum, 0, 3));
		Row report3Name = sheet.createRow(rowNum);
		report3Name.createCell(0).setCellValue("Most purchased customer");
		rowNum++;
		Row header3 = sheet.createRow(rowNum);
		header3.createCell(0).setCellValue("CusId");
		header3.createCell(1).setCellValue("CusName");
		header3.createCell(2).setCellValue("Total seat purchased");
		rowNum++;
		Iterator<String[]> report3List = report3.iterator();
		while (report3List.hasNext()) {
			Row row = sheet.createRow(rowNum++);
			ArrayList<String> item = new ArrayList<>(Arrays.asList(report3List.next()));
			row.createCell(0).setCellValue(item.get(0));
			row.createCell(1).setCellValue(item.get(1));
			row.createCell(2).setCellValue(item.get(2));
		}
	}
